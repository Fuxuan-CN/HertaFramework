...
namespace Herta.Security.MiddlewarePolicy.TimeSpeedLimitSecurityPolicy
{
    // 基于时间的速率限制
    public class TimeSpeedLimitSecurityPolicy : ISecurityPolicy
    {
        ...
        public TimeSpeedLimitSecurityPolicy()
        {
            _maxRequestsPerSecond = 100;
        }
        
        public TimeSpeedLimitSecurityPolicy(int maxRequestsPerSecond = 100)
        {
        ...
        {
            string result = $"sorry, but you have exceeded the maximum number of requests per second. limit is : {_maxRequestsPerSecond} per {_timeWindow.TotalSeconds} seconds.";
