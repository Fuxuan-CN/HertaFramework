...
namespace Herta.Exceptions.HttpException
{
    ...
    public sealed class HttpException : HertaBaseException
    // raise when there is an error with the http request or response
    {
        ...
        public HttpException(int statusCode, string? message = null, Exception? cause = null)
            : base(message ?? string.Empty, cause)
        {
            StatusCode = statusCode;
            Detail = null;
            ErrMessage = message;
