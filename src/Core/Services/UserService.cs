...
namespace Herta.Core.Services.UserService
{
    ...
    public class UserService : IUserService
    {
        ...
        public async Task<User> RegisterAsync(string username, string password)
        {
            ...
            {
                throw new HttpException(StatusCodes.Status400BadRequest, "用户名已存在.");
            }
        ...
        public async Task<User> LoginAsync(string username, string password)
        {
            ...
            {
                throw new HttpException(StatusCodes.Status401Unauthorized, "用户名或密码错误.");
            }
        ...
        public async Task ChangePasswordAsync(string username, string oldPassword, string newPassword)
        {
            ...
            if (user == null)
            {
                throw new HttpException(StatusCodes.Status404NotFound, "用户不存在.");
            }

            if (!BCrypt.Net.BCrypt.Verify(oldPassword, user.PasswordHash))
            {
                throw new HttpException(StatusCodes.Status400BadRequest, "旧密码错误.");
            }
        ...
        public async Task DeleteUserAsync(DeleteUserForm form)
        {
            ...
            {
                throw new HttpException(StatusCodes.Status401Unauthorized, "密码错误.");
            }
        }
        ...
        private async Task CheckUserFreezed(string username)
        {
            ...
            if (user != null)
            {
                throw new HttpException(StatusCodes.Status403Forbidden, "用户已被冻结.");
            }
        }
