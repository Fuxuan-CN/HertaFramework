...
public class UserInfoService : IUserInfoService
{
    ...
    public async Task<UserInfo> GetUserInfoAsync(int userId)
    {
        ...
        if (userInfo == null)
        {
            throw new HttpException(StatusCodes.Status404NotFound, "user info not found.");
        }
        return userInfo;
    }
    ...
    public async Task<bool> PartialUpdateUserInfoAsync(UpdateInfoForm updateInfoForm)
    {
        ...
        foreach (var (key, value) in updateInfoForm.UpdateInfo)
        {
            ...
            if (property == null)
            {
                throw new HttpException(StatusCodes.Status400BadRequest, $"field '{key}' not found.");
            }
            ...
            switch (propertyType)
            {
                ...
                case Type t when (t == typeof(string[])):
                    ...
                    break;
                default:
                    throw new HttpException(StatusCodes.Status400BadRequest, $"field '{key}', type: '{propertyType.Name}' not supported.");
            }
        }
        ...
        {
            throw new HttpException(StatusCodes.Status500InternalServerError, "update failed.", ex);
        }
    }
}
